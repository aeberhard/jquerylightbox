 ...
 */
	if ( isset($_POST['function']) and ($_POST['function']=='save') ) {
		...
		if (($fh = fopen($rxa_jqlightbox['path'].'/'.$rxa_jqlightbox['name'].'.ini', 'w')) === FALSE) {
			$rxa_jqlightbox['meldung'] = $rxa_jqlightbox['i18n']->msg('error_save',$rxa_jqlightbox['path'].'/'.$rxa_jqlightbox['name'].'.ini');
		} else {
			@fwrite($fh, $line);
			@fclose($fh);	
			$rxa_jqlightbox['meldung'] = $rxa_jqlightbox['i18n']->msg('msg_saved');
		}
	}
 ...
 */
	if (($lines = file($rxa_jqlightbox['path'].'/'.$rxa_jqlightbox['name'].'.ini')) === FALSE) {
		$rxa_jqlightbox['meldung'] = $rxa_jqlightbox['i18n']->msg('error_read',$rxa_jqlightbox['path'].'/'.$rxa_jqlightbox['name'].'.ini');
	} else {
		$va = explode(',', trim($lines[0]));
		$allcats = trim($va[0]);
		$subcats = trim($va[1]);
		$rxa_jqlightbox['cats'] = unserialize(trim($lines[1]));
	}
	if (!is_array($rxa_jqlightbox['cats'])){
		$rxa_jqlightbox['cats'] = array();
	}
 ...
 */
	$cat_ids[] = '';
	$cat='0';
   $rxa_jqlightbox['catselectcount'] = 0;
   ...
   $select_cats = "\n";
	if ($cats = OOCategory::getRootCategories())
	{
		foreach( $cats as $cat)
		{
			add_cat_options($rxa_jqlightbox, $select_cats, $cat, $cat_ids);
		}
	}
	//Artikel aus dem Root ebenso in die Auswahl (selectbox) übernehmen
	add_rootart_options($rxa_jqlightbox, $select_cats, $clang);
	
	$selsize = $rxa_jqlightbox['catselectcount'] / 3;
...
<form action="index.php?page=<?php echo $rxa_jqlightbox['name']; ?>" method="post">
...
<?php
	if ($rxa_jqlightbox['meldung']<>'') echo $rxa_jqlightbox['meldung'].'<br />';
	echo $rxa_jqlightbox['i18n']->msg('text_settings_intro');
?>
        <br /><br />

        <input type="checkbox" id="allcats" name="allcats" value="1" <?php if ($allcats == "1") echo "checked"; ?> />
		  <label for="allcats"><?php echo $rxa_jqlightbox['i18n']->msg('text_settings_allcats'); ?></label>
        <br />

        <input type="checkbox" id="subcats" name="subcats" value="1" <?php if ($subcats == "1") echo "checked"; ?> />
		  <label for="subcats"><?php echo $rxa_jqlightbox['i18n']->msg('text_settings_subcats'); ?></label>
        <br /><br />

<?php echo $rxa_jqlightbox['i18n']->msg('text_settings_help'); ?>

<?php
		echo $select_cats;
?>
        <br /><br />
        <input type="submit" value="<?php echo $rxa_jqlightbox['i18n']->msg('button_save'); ?>" />
